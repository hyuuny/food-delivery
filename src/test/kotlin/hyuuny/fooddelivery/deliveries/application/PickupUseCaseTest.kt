...
class PickupUseCaseTest : BehaviorSpec({
    ...
    Given("라이더가 음식을 픽업하면서") {
        ...
        coEvery { repository.updatePickupTime(any()) } returns Unit

        When("현재 음식을 픽업할 수 있는 상태라면") {
            useCase.pickup(delivery.id!!, { order }, { rider })

            Then("픽업할 수 있다.") {
                coVerify { repository.updatePickupTime(any()) }
            }
        }
