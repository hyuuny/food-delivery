...
import hyuuny.fooddelivery.carts.presentation.api.v1.request.AddItemAndOptionRequest
...
class AddItemToCartCartUseCaseTest : BehaviorSpec({
    ...
    Given("장바구니에 메뉴를 등록하면") {
        ...
        `when`("선택한 품목과 옵션에 맞게") {
            val result = useCase.addItemToCart(userId, request) { store }
        ...
        `when`("존재하는 장바구니에 다른 매장의 메뉴를 담으면") {
            ...
            then("장바구니에는 같은 매장의 메뉴만 담을 수 있다는 메세지가 반환된다.") {
                val ex = shouldThrow<IllegalArgumentException> {
                    useCase.addItemToCart(userId, request.copy(storeId = 2)) { store }
                }
        ...
        `when`("옵션이 하나라도 없으면") {
            ...
            then("품목 옵션은 필수값이라는 메세지가 반환된다.") {
                val ex = shouldThrow<IllegalArgumentException> {
                    useCase.addItemToCart(userId, badRequest) { store }
                }
